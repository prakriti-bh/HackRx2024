...
def load_airoboros_model(device='cuda'):
    model_name = "jondurbin/airoboros-l2-7b-gpt4-1.4.1"  # Smaller 7B model
...
def summarize_with_airoboros(model, tokenizer, text, max_length=150):
    chunks = chunk_text(text)
    summaries = []
    for chunk in chunks:
        prompt = f"Summarize the following text concisely, focusing on key business points:\n\n{chunk}\n\nSummary:"
...
def generate_script_with_airoboros(model, tokenizer, summary, max_length=500):
    prompt = f"Based on the following summary, create a professional script for a corporate video presentation:\n\n{summary}\n\nScript:"
    inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
    with torch.no_grad():
        outputs = model.generate(**inputs, max_new_tokens=max_length, temperature=0.7)
...
def extract_key_points(model, tokenizer, summary, n=5):
    prompt = f"Extract {n} key business points from the following summary:\n\n{summary}\n\nKey points:"
    inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
    with torch.no_grad():
...
def generate_corporate_prompts(key_points):
    prompts = []
    for point in key_points:
        prompt = f"Create a professional, corporate-style image depicting {point}. The image should be suitable for a business presentation, with clean lines, modern aesthetics, and a professional color scheme. Include business people, office settings, or relevant business icons where appropriate. High quality, detailed, 4K resolution."
...
def generate_quiz_with_airoboros(model, tokenizer, script, num_questions=5):
    ...
    prompt = f"Based on the following corporate video script, generate {num_questions} professional quiz questions with their answers:\n\n{script}\n\nQuiz:"
    inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
    with torch.no_grad():
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Automated Corporate Video Generation from Text")
...
def main(args):
    ...
    summary = summarize_with_airoboros(model, tokenizer, text, max_length=args.summary_length)
    script = generate_script_with_airoboros(model, tokenizer, summary, max_length=args.script_length)
    key_points = extract_key_points(model, tokenizer, summary, n=args.num_images)

    print("Generating prompts and images...")
    prompts = generate_corporate_prompts(key_points)
    ...
    quiz = generate_quiz_with_airoboros(model, tokenizer, script, num_questions=args.num_questions)
